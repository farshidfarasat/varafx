...
- Local run: `npm run dev`. Production deploy: `npm run deploy` (wrangler deploy) — deployment is a production mutation: PR-first, never push directly to `main`, and deploy only on the owner's release decision.
